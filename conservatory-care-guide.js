...
function card(name){let c=care(name);return `<div class="care-guide"><p class="eyebrow">CARE COMPASS</p><div class="care-grid"><span>☀️ <b>Light</b><br>${c[0]}</span><span>💧 <b>Water</b><br>${c[1]}</span><span>💨 <b>Humidity</b><br>${c[2]}</span><span>🪴 <b>Soil</b><br>${c[3]}</span></div><p class="quiet">${c[4]}</p></div>`}
function enhance(){let grid=document.getElementById('plantGrid');if(!grid)return;grid.querySelectorAll('.plant').forEach(node=>{let h=node.querySelector('h3'),name=h?.textContent?.trim();if(name&&!node.querySelector('.care-guide'))h.insertAdjacentHTML('afterend',card(name))})}
function modalCare(){let m=document.getElementById('modalContent');if(!m||m.querySelector('.care-guide'))return;let tag=m.querySelector('.eyebrow')?.textContent||'';if(!/SPECIMEN JOURNAL/i.test(tag))return;let h=m.querySelector('h2'),name=h?.textContent?.trim();if(name)h.insertAdjacentHTML('afterend',card(name))}
